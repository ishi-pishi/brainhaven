...
// The `UpdateUserProgress` mutation has an optional argument of type `UpdateUserProgressVariables`:
...
// Variables can be defined inline as well.
const { data } = await updateUserProgress({ currencyBalance: ..., currentStreak: ..., lastGoalMetDate: ..., dailyGoalMinutes: ..., });
...
// Variables can be defined inline as well.
const ref = updateUserProgressRef({ currencyBalance: ..., currentStreak: ..., lastGoalMetDate: ..., dailyGoalMinutes: ..., });
// Since all variables are optional for this mutation, you can omit the `UpdateUserProgressVariables` argument.
const ref = updateUserProgressRef();
...
// Variables can be defined inline as well.
const { data } = await createReward({ name: ..., cost: ..., isOneTime: ..., });
...
// Variables can be defined inline as well.
const ref = createRewardRef({ name: ..., cost: ..., isOneTime: ..., });
...
## DeleteReward
You can execute the `DeleteReward` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
deleteReward(vars: DeleteRewardVariables): MutationPromise<DeleteRewardData, DeleteRewardVariables>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `MutationRef` function.
```typescript
deleteReward(dc: DataConnect, vars: DeleteRewardVariables): MutationPromise<DeleteRewardData, DeleteRewardVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the deleteRewardRef:
```typescript
const name = deleteRewardRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `DeleteReward` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `DeleteRewardData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
export interface DeleteRewardData {
  reward_delete?: Reward_Key | null;
}
```
### Using `DeleteReward`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, deleteReward, DeleteRewardVariables } from '@dataconnect/generated';

// The `DeleteReward` mutation requires an argument of type `DeleteRewardVariables`:
const deleteRewardVars: DeleteRewardVariables = {
  id: ..., 
};

// Call the `deleteReward()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await deleteReward(deleteRewardVars);
// Variables can be defined inline as well.
...
console.log(data.reward_delete);

// Or, you can use the `Promise` API.
deleteReward(deleteRewardVars).then((response) => {
  const data = response.data;
  console.log(data.reward_delete);
});
```

### Using `DeleteReward`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, deleteRewardRef, DeleteRewardVariables } from '@dataconnect/generated';
...
// Call the `deleteRewardRef()` function to get a reference to the mutation.
const ref = deleteRewardRef(deleteRewardVars);
// Variables can be defined inline as well.
const ref = deleteRewardRef({ id: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = deleteRewardRef(dataConnect, deleteRewardVars);
...
// Variables can be defined inline as well.
const { data } = await createUser({ displayName: ..., });
...
// Variables can be defined inline as well.
const ref = createUserRef({ displayName: ..., });
...
// Variables can be defined inline as well.
const { data } = await createStudySession({ subjectId: ..., startTime: ..., endTime: ..., workBlockMs: ..., breakBlockMs: ..., intendedCycles: ..., productivityRating: ..., reflections: ..., earnedCurrency: ..., });
...
// Variables can be defined inline as well.
const ref = createStudySessionRef({ subjectId: ..., startTime: ..., endTime: ..., workBlockMs: ..., breakBlockMs: ..., intendedCycles: ..., productivityRating: ..., reflections: ..., earnedCurrency: ..., });
...
console.log(data.studySession_deleteMany);
